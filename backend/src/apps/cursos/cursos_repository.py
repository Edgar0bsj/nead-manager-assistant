...
class CursoRepository:
    ...
    def find_all(self, name_entity: Optional[str] = None) -> list[CursosModel]:
        # return self.session.query(AlteracaoCodigoExternoModel).all()

        query = self.session.query(CursosModel)

        if name_entity:
            query = query.filter(CursosModel.name.ilike(f"%{name_entity}%"))
    ...
    def find_all_by(
        ...
        query = self.session.query(CursosModel).filter(CursosModel.deleted_at.is_(None))
